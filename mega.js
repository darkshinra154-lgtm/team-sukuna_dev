...
password: 'k6hm_HktD4r845d', // Replace with your Mega password
...
} catch (err) {reject(err)}// Reject if any error occurs during the upload process
});
};
...
if (err) {reject(err)} else {resolve(buffer)} // Return the file buffer
});
});
} catch (err) {
reject(err);
